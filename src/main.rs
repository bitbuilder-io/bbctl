...
fn cli_handler(cli: Cli) -> AppResult<()> {
    match cli.command {
        Some(Commands::Init { name }) => {
            println!(
                "Initializing BitBuilder Cloud project: {}",
        ...
        Some(Commands::Instances { action }) => match action {
            ...
            InstancesCommands::Create {
                ...
                println!(
                    "Creating instance '{}' with provider '{}' in region '{}'",
                    name, provider, region
                );
                println!(
                    "Resources: CPU: {}, Memory: {} GB, Disk: {} GB",
                    cpu.unwrap_or(1),
                    memory.unwrap_or(2),
                    disk.unwrap_or(10)
                );
        ...
        Some(Commands::Volumes { action }) => match action {
            ...
            VolumesCommands::Create { name, size, region } => {
                println!(
                    ...
                    name,
                    size,
                    region.unwrap_or_else(|| "nyc".to_string())
                );
            }
        ...
        Some(Commands::Networks { action }) => match action {
            ...
            NetworksCommands::Disconnect { id, instance } => {
                println!(
                    "Disconnecting instance '{}' from network '{}'",
                    instance, id
                );
            }
                    ...
                    Err(e) => {
                        ...
                        if output.status.success() || output.status.code() == Some(255) {
                            ...
                            if stdout.contains("VyOS") {
                                ...
                                return Err(format!(
                                    "Connected but did not receive VyOS welcome message"
                                )
                                .into());
                            }
                        } else {
