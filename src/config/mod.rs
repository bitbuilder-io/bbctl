...
pub fn read_config_file(file_name: &str) -> Result<String> {
    ...
    fs::read_to_string(&path).context(format!("Failed to read config file: {}", path.display()))
}
...
pub fn write_config_file(file_name: &str, content: &str) -> Result<()> {
    ...
    fs::write(&path, content).context(format!("Failed to write config file: {}", path.display()))
}
...
pub fn init_config() -> Result<()> {
    ...
    if !settings_path.exists() {
        ...
        fs::write(&settings_path, toml).context(format!(
    ...
    if !providers_path.exists() {
        ...
        let toml =
            toml::to_string_pretty(&providers).context("Failed to serialize empty providers")?;
        fs::write(&providers_path, toml).context(format!(
            "Failed to write providers file: {}",
    ...
    if !credentials_path.exists() {
        ...
        fs::write(&credentials_path, toml).context(format!(
