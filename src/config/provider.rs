...
use crate::models::provider::{ProviderConfig, ProviderType, Region};
...
impl Providers {
    ...
    pub fn load() -> Result<Self> {
        ...
        let providers: Providers =
            toml::from_str(&content).context("Failed to parse providers TOML")?;
    ...
    pub fn save(&self) -> Result<()> {
        ...
        // Serialize to TOML
        let content = toml::to_string_pretty(self).context("Failed to serialize providers")?;

        // Write to file
        write_config_file(PROVIDERS_FILE, &content).context("Failed to write providers file")?;
    ...
    pub fn remove_provider(&mut self, name: &str) -> Result<()> {
        ...
        self.regions
            .retain(|_, region| region.provider.to_string() != name);
