...
impl ProxmoxClient {
        ...
        match &self.config.auth {
            ...
            ProxmoxAuth::ApiToken {
                ...
                if response.status().is_success() {
                    self.connected = true;
                    info!(
                        "Successfully authenticated to Proxmox with API token: {}",
                        self.config.host
                    );
                    Ok(())
                } else {
        ...
        match &self.config.auth {
            ProxmoxAuth::UserPass { .. } => {
                ...
                if let Some(ticket) = &self.ticket {
                    request_builder =
                        request_builder.header("Cookie", format!("PVEAuthCookie={}", ticket));
    ...
    pub async fn get_vms(&mut self, node: &str) -> Result<serde_json::Value> {
        self.api_call(&format!("nodes/{}/qemu", node), "GET", None)
            .await
    }
    ...
    pub async fn create_vm(
        ...
        self.api_call(&format!("nodes/{}/qemu", node), "POST", Some(params))
            .await
    }

    /// Delete a VM
    pub async fn delete_vm(&mut self, node: &str, vmid: u64) -> Result<serde_json::Value> {
        self.api_call(&format!("nodes/{}/qemu/{}", node, vmid), "DELETE", None)
            .await
    }

    /// Get storage information
    pub async fn get_storage(&mut self, node: &str) -> Result<serde_json::Value> {
        self.api_call(&format!("nodes/{}/storage", node), "GET", None)
            .await
    }
}
