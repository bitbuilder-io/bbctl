...
impl VolumeService {
    ...
    pub async fn create_volume(
        ...
        info!("Creating volume '{}' with size {} GB in region '{}'", name, size_gb, region);
    ...
    pub async fn attach_volume(&mut self, volume_id: &Uuid, instance_id: &Uuid, device: &str) -> Result<()> {
        // Get the volume
        let volume = self.storage.get_volume_mut(volume_id)
        ...
        info!("Attached volume {} to instance {} at {}", volume_id, instance_id, device);
        Ok(())
    }
    ...
    pub async fn detach_volume(&mut self, volume_id: &Uuid) -> Result<()> {
        ...
        if volume.attached_to.is_none() {
            return Err(anyhow!("Volume {} is not attached to any instance", volume_id));
        }
    ...
    pub async fn delete_volume(&mut self, volume_id: &Uuid) -> Result<()> {
        ...
        // Check if attached
        if volume.attached_to.is_some() {
            return Err(anyhow!("Cannot delete attached volume {}. Detach it first.", volume_id));
        }
