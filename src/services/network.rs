...
impl NetworkService {
    ...
    pub async fn create_network(
        ...
        info!(
            "Creating network '{}' with CIDR {} in region '{}'",
            name, cidr, region
        );
    ...
    pub async fn connect_instance(
        ...
        if network.instances.contains(instance_id) {
            return Err(anyhow!(
                "Instance {} is already connected to network {}",
                instance_id,
                network_id
            ));
        }
        ...
        network
            .allocate_ip(ip, *instance_id)
            .map_err(|e| anyhow!(e))?;

        info!(
            "Connected instance {} to network {} with IP {}",
    ...
    pub async fn disconnect_instance(
        ...
        if !network.instances.contains(instance_id) {
            return Err(anyhow!(
                "Instance {} is not connected to network {}",
                instance_id,
                network_id
            ));
        }
        ...
        info!(
            "Disconnected instance {} from network {}",
    ...
    pub async fn delete_network(&mut self, network_id: &Uuid) -> Result<()> {
        ...
        if !network.instances.is_empty() {
            return Err(anyhow!(
                "Cannot delete network {} with connected instances. Disconnect them first.",
                network_id
            ));
        }
