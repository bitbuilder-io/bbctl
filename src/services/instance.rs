...
use crate::models::instance::{Instance, InstanceNetwork, InstanceSize, InstanceStatus};
...
impl InstanceService {
    ...
    pub async fn create_proxmox_instance(
        ...
        info!(
            "Creating Proxmox instance '{}' in region '{}'",
            name, region
        );
    ...
    pub async fn start_instance(&mut self, id: &Uuid) -> Result<()> {
        ...
        match provider {
            ProviderType::VyOS => {
                ...
                let result = client
                    .execute_ssh_command(&format!("start vm {}", provider_id))
                    .await;
    ...
    pub async fn stop_instance(&mut self, id: &Uuid) -> Result<()> {
        ...
        match provider {
            ProviderType::VyOS => {
                ...
                // Example: Send commands over SSH
                let result = client
                    .execute_ssh_command(&format!("stop vm {}", provider_id))
                    .await;
    ...
    pub async fn delete_instance(&mut self, id: &Uuid) -> Result<()> {
        ...
        match provider {
            ProviderType::VyOS => {
                ...
                // Example: Send commands over SSH
                let result = client
                    .execute_ssh_command(&format!("delete vm {}", provider_id))
                    .await;
